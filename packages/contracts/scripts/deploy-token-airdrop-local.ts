...
let membershipMerkleWitness: WhitelistMembershipMerkleWitness = await whitelistTree.getSiblingPath(leafIndex.toBigInt(), true);
...
const newSlot = Mina.activeInstance.getNetworkState().globalSlotSinceGenesis.add(5);
(Mina.activeInstance as any).setGlobalSlot(newSlot);
...
tx = await Mina.transaction(
    ...
    () => {
        ...
        basicTokenZkApp.transferToken(redeemAccountZkAppAddress, to, UInt64.from(1));
    }
);
await ctx.submitTx(tx, {
    feePayerKey: redeemAccountZkAppKey,
