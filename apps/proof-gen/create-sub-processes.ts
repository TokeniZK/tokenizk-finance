...
export type SubProcessCordinator = {
    ...
    clientPresaleContractCall: (proofPayload: ProofPayload<any>, sendCallBack?: (x: any) => void) => Promise<ProofPayload<any>>,

    clientFairsaleContractCall: (proofPayload: ProofPayload<any>, sendCallBack?: (x: any) => void) => Promise<ProofPayload<any>>,

    clientPrivatesaleContractCall: (proofPayload: ProofPayload<any>, sendCallBack?: (x: any) => void) => Promise<ProofPayload<any>>
};
...
const CircuitName_Client_TokeniZkPresale = 'Client_TokeniZkPresale';
const CircuitName_Client_TokeniZkFairsale = 'Client_TokeniZkFairsale';
const CircuitName_Client_TokeniZkPrivatesale = 'Client_TokeniZkPrivatesale';
...
const cnt_Client_TokeniZkPresale = config.cnt_Client_TokeniZkPresale;
const cnt_Client_TokeniZkFairsale = config.cnt_Client_TokeniZkFairsale;
const cnt_Client_TokeniZkPrivatesale = config.cnt_Client_TokeniZkPrivatesale;
...
let clientPresaleContractCallTimes = 0;
let clientFairsaleContractCallTimes = 0;
let clientPrivatesaleContractCallTimes = 0;
...
export const createSubProcesses = async () => {
    ...
    let workerMap = new Map<string, { worker: Worker, status: WorkerStatus, type: string }[]>([
        ...
        [CircuitName_Client_TokeniZkPresale, []],
        [CircuitName_Client_TokeniZkFairsale, []],
        [CircuitName_Client_TokeniZkPrivatesale, []]
    ]);
    ...
    /* 
        ...
        createCircuitProcessor(cnt_Client_TokeniZkFairsale, CircuitName_Client_TokeniZkFairsale);
        createCircuitProcessor(cnt_Client_TokeniZkPrivatesale, CircuitName_Client_TokeniZkPrivatesale);
    */

    createCircuitProcessor(cnt_Client_TokeniZkPresale, CircuitName_Client_TokeniZkPresale);
    ...
    return {
        ...
        clientPresaleContractCall: async (proofPayload: ProofPayload<any>, sendCallBack?: any) => {
            return await new Promise(
                (
                    resolve: (payload: ProofPayload<any>) => any,
                    reject: (err: any) => any | any
                    ...
                    generateProof(workerMap.get(CircuitName_Client_TokeniZkPresale)!, msg, fromJsonFn, resolve, reject, sendCallBack);
                }
            ).catch(e => {
                console.error(e);
            });
        ...
        clientFairsaleContractCall: async (proofPayload: ProofPayload<any>, sendCallBack?: any) => {
            return await new Promise(
                (
                    resolve: (payload: ProofPayload<any>) => any,
                    reject: (err: any) => any | any
                    ...
                    generateProof(workerMap.get(CircuitName_Client_TokeniZkFairsale)!, msg, fromJsonFn, resolve, reject, sendCallBack);
                }
            ).catch(e => {
                console.error(e);
            });
        ...
        clientPrivatesaleContractCall: async (proofPayload: ProofPayload<any>, sendCallBack?: any) => {
            return await new Promise(
                (
                    resolve: (payload: ProofPayload<any>) => any,
                    reject: (err: any) => any | any
                    ...
                    generateProof(workerMap.get(CircuitName_Client_TokeniZkPrivatesale)!, msg, fromJsonFn, resolve, reject, sendCallBack);
                }
            ).catch(e => {
                console.error(e);
            });
...
function getFreeWorker(
    ...
    if (worker === undefined) {
        ...
        } else if (worker.type == CircuitName_Client_TokeniZkPresale) {
            // by return, due to the last process need time to release memory(about wasm32, don't know why, but occurs), or else it will fail!
            logger.info(`worker.type: ${worker.type}, clientPresaleContractCallTimes:${clientPresaleContractCallTimes}, workerIndex: ${clientPresaleContractCallTimes % workers.length}`);

            worker = workers.at(clientPresaleContractCallTimes % workers.length);
            clientPresaleContractCallTimes++;
        } else if (worker.type == CircuitName_Client_TokeniZkFairsale) {
            // by return, due to the last process need time to release memory(about wasm32, don't know why, but occurs), or else it will fail!
            logger.info(`worker.type: ${worker.type}, clientFairsaleContractCallTimes:${clientFairsaleContractCallTimes}, workerIndex: ${clientFairsaleContractCallTimes % workers.length}`);

            worker = workers.at(clientFairsaleContractCallTimes % workers.length);
            clientFairsaleContractCallTimes++;
        } else if (worker.type == CircuitName_Client_TokeniZkPrivatesale) {
            // by return, due to the last process need time to release memory(about wasm32, don't know why, but occurs), or else it will fail!
            logger.info(`worker.type: ${worker.type}, clientPrivatesaleContractCallTimes:${clientPrivatesaleContractCallTimes}, workerIndex: ${clientPrivatesaleContractCallTimes % workers.length}`);

            worker = workers.at(clientPrivatesaleContractCallTimes % workers.length);
            clientPrivatesaleContractCallTimes++;
        }
